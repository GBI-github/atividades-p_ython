class Animal:
    def __init__(self, falar):
        self.falar = falar


class Gato(Animal):
    def __init__(self, falar, cor, idade):
        super().__init__(falar)
        self.cor = cor
        self.idade = idade


class Cachorro(Animal):
    def __init__(self, falar, cor, idade):
        super().__init__(falar)
        self.cor = cor
        self.idade = idade


def descrevendo_animal(animale):
    print(f"Este animal diz: {animale.falar}")
    if isinstance(animale, Gato) or isinstance(animale, Cachorro):
        print(f"Cor do pelo: {animale.cor}")
        print(f"Idade: {animale.idade}")


# Exemplo de uso
animal_aleatorio = Animal("auuu")
gato_cinza = Gato("miau miau", "cinza", 3)
cachorro_caramelo = Cachorro("au au au", "caramelo", 5)

descrevendo_animal(animal_aleatorio)
descrevendo_animal(gato_cinza)
descrevendo_animal(cachorro_caramelo)
